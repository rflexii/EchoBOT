...
async function handleChat(req: NextRequest) {
  ...
  // Update visitor info
  if (body.visitor?.name && body.visitor.name !== conv.visitorName) { await db.update(conversations).set({ visitorName: body.visitor.name, visitorEmail: body.visitor.email ?? conv.visitorEmail, visitorPhone: body.visitor.phone ?? conv.visitorPhone }).where(eq(conversations.id, conv.id)); conv.visitorName = body.visitor.name; }
  ...
  // ALWAYS create/update lead (even without name, to capture the conversation)
  ...
  if (wantsEscalation) {
    ...
    if (!existingTicket) {
      var [ticket] = await db.insert(tickets).values({ ticketNumber: ticketNumber(), conversationId: conv.id, subject: "Visitor requested escalation", description: body.message.slice(0, 300), priority: "medium", visitorName: conv.visitorName, visitorEmail: conv.visitorEmail, visitorPhone: conv.visitorPhone }).returning();
