    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 text-sm">
          ...
          <div><span className="text-slate-400">Priority:</span> <span className="capitalize">{ticket.priority || "medium"}</span></div>
          <div><span className="text-slate-400">Created:</span> {fmtDate(ticket.createdAt)}</div>
          {ticket.description && <div className="mt-2 rounded-lg bg-slate-50 p-3"><span className="text-slate-400">Description:</span><p className="mt-1 whitespace-pre-wrap">{ticket.description}</p></div>}
        </div>
